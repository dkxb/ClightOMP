...
Ltac pf_cleanup :=
    repeat match goal with
           ...
           | [H1: ThreadPool.containsThread ?TP ?M,
             ...
             assert (H1 = H2) by (by eapply ThreadPool.cnt_irr); subst H2
           | [H1: FinPool.containsThread ?TP ?M, H2: FinPool.containsThread ?TP ?M |- _] =>
             assert (H1 = H2) by (by eapply proof_irr); subst H2
           | [H1: FinPool.containsThread ?TP ?M,
                  H2: FinPool.containsThread (@FinPool.updThreadC _ ?TP _ _) ?M |- _] =>
             apply FinPool.cntUpdateC' in H2;
             assert (H1 = H2) by (by eapply FinPool.cnt_irr); subst H2
           | [H1: FinPool.containsThread ?TP ?M,
                  H2: FinPool.containsThread (@FinPool.updThread _ ?TP _ _ _) ?M |- _] =>
             apply FinPool.cntUpdate' in H2;
             assert (H1 = H2) by (by eapply FinPool.cnt_irr); subst H2
           | [H1: ThreadPool.containsThread ?TP ?M, H2: FinPool.containsThread ?TP ?M |- _] =>
             assert (H1 = H2) by (by eapply proof_irr); subst H2
           end.
End Tactics.
