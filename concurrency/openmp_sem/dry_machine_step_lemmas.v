...
Require Import VST.concurrency.openmp_sem.sepcomp. Import SepComp.
...
Module StepLemmas.
  ...
  Section StepLemmas.
    ...
    Instance ClightDryMachineInst : MachineSig := (@DryHybridMachine.DryHybridMachineSig _ _).
  ...
  Lemma step_mem_compatible:
    forall U tr tp ttree m U' tr' tp' ttree' m'
      (Hstep: MachStep (U, tr, tp, ttree) m (U', tr', tp', ttree') m'),
      mem_compatible tp m.
  Proof.
    intros.
  ...
  Lemma gsoThreadR_step:
    forall tp tp' m m' i j U tr tr' tre tre'
      (Hneq: i <> j)
  ...
  Proof.
    ...
    inversion Hstep; simpl in *; subst;
      try (inversion Htstep); subst; inversion HschedN; subst; Tactics.pf_cleanup;
        try (by rewrite <- FinPool.gThreadCR with (cntj' := pfj'));
        try (rewrite FinPool.gLockSetRes);
        try (rewrite @FinPool.gsoThreadRes); eauto.
    (* rewrite FinPool.gsoAddRes. FinPool.gsoThreadRes; now auto.
    rewrite FinPool.gRemLockSetRes FinPool.gsoThreadRes; now auto.
  Qed. *)
  Admitted.
  ...
  Lemma permission_at_step:
    forall tp tp' m m' i j U tr tr' tre tre'
      ...
      (Hcomp: mem_compatible tp m)
      (Hcomp': mem_compatible tp' m')
      (Hstep: MachStep (i :: U, tr, tp, tre) m (U,tr',tp', tre') m') b ofs,
  ...
  (* TODO fix safety definitions *)
  (* Lemma safeC_invariant:
    forall tpc trc trec mc n
      (Hn: n > 0)
      (Hsafe: forall U,
          HybridCoarseMachine.csafe (U,trc,tpc, trec) mc n),
      invariant tpc.
  Proof.
    intros.
    specialize (Hsafe [:: 1]).
  ...
  Lemma step_schedule:
    forall tpc tpc' mc mc' i U1 U2 U1' tr tr' tre tre'
      (Hstep: MachStep (i :: U1, tr, tpc, tre) mc (U2, tr ++ tr', tpc', tre') mc'),
      exists U2',
      MachStep (i :: U1', tr, tpc, tre) mc (U2', tr ++ tr', tpc', tre') mc'.
  Proof.
    intros.
...
(** ** Definition of internal steps *)
  Module InternalSteps.
    ...
    Section InternalSteps.
      ...
      (** Take an instance of the Dry Machine *)
    Instance ClightDryMachineInst : MachineSig := (@DryHybridMachine.DryHybridMachineSig ge _).
    ...
    Proof.
      ...
      - subst.
        ...
        simpl.
        erewrite @FinPool.gsoThreadRes;
          now eauto.
        erewrite @gsoThreadCode with (cntj := ctn); eauto.
        Tactics.pf_cleanup. auto.
        simpl.
        rewrite FinPool.updThread_updThreadC_comm; auto.
        admit.
      - subst.
        inversion H; subst.
        do 2 right.
        ...
        + simpl in *.
          ...
          simpl.
          erewrite @FinPool.gsoThreadRes with (cntj' := cntj');
            now eauto.
        + Tactics.pf_cleanup.
          simpl in *.
          unfold HybridMachine.DryHybridMachine.add_block in *.
          simpl in *.
          rewrite FinPool.updThread_comm; eauto.
          erewrite @FinPool.gsoThreadRes with (cntj' := cntj') by eauto.
          reflexivity.
          (* inversion Hperm; subst. *)
        ...
        (*   } *)
        (*   { simpl. *)
        (*     unfold FinPool.lr_valid. *)
        (*     intros. *)
        (*     rewrite! FinPool.gsoThreadLPool. *)
        (*     destruct Hinv. *)
        (*     specialize (lockRes_valid0 b ofs). *)
        (*     simpl in lockRes_valid0. *)
        (*     destruct (FinPool.lockRes tp (b,ofs)); *)
        (*       now auto. *)
        (*   } *)
          Unshelve. auto.
    Admitted.
    ...
    (* FIXME change to OpenMP version of addThread *)
    ...
    Proof.
      ...
      - destruct Hinit; subst.
        ...
        eapply restrPermMap_irr'.
        simpl.
        erewrite @FinPool.gsoAddRes; now eauto.
    ...
    (* Proof.
      ...
      - subst.
        ...
        erewrite restrPermMap_irr with (m2 := m') (P2 := (Hcmpt _ ctn).1); eauto.
        simpl.
        erewrite FinPool.gRemLockSetRes;
          now eauto.
      ...
      - subst.
        ...
        eapply restrPermMap_irr'.
        simpl.
        erewrite @FinPool.gRemLockSetRes; now eauto.
        eapply ThreadPoolWF.remLock_inv; eauto.
    ...
    Arguments internal_step {ge} {tid} {tp} {m}.
    Arguments internal_execution {ge}.
...
Module StepType.
  ...
  Definition ctlType (code : @finThreadPool.ctl semC) (m : Mem.mem) (st : StepType) : Prop :=
    match code with
  ...
  (* commenting out FineMachineInternal for now, as we want to prove DRF on coarse machine. *)
